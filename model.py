    ...
    for member_index in range(len(staff)):
        ...
        for shift_index, shift in enumerate(shifts):
            ...
            if shift.kind == "largo":
                ...
                # Horizon boundary: if the next day is outside the loaded period,

                # do not force an assignment to a shift that was not created.
            ...
            if shift.kind == "noche":
                ...
                # Horizon boundary: allow a night at the beginning of the loaded

                # period because its preceding long shift may belong to prior data.
...
def optimize_schedule(
    ...
    available_hours = sum(
        ...
        staff_hour_limit(member, rules) for member in staff
    ...
    # Coverage: every shift must be assigned exactly the required number of staff.
    ...
    # Optional daily rule: staff cannot work more than one shift on the same day.
    ...
    # Rest rules: prevent overlaps and enforce rest between consecutive shifts.
    ...
    # Cuarto turno: usually a preferred cycle. Keep hard pairing optional because

    # absences and vacations should create replaceable gaps, not infeasibility.
    ...
    # Max hours: staff cannot exceed personal and optional weekly hour limits.
    ...
    for member_index, member in enumerate(staff):
        ...
        assigned_shift_count = model.NewIntVar(
            ...
            f"assigned_shift_count_{member.id}",
    ...
    for role in roles:
        ...
        max_assigned_shifts = model.NewIntVar(

            0,

            len(shifts),

            f"max_assigned_shifts_{role}",

        )

        min_assigned_shifts = model.NewIntVar(

            0,

            len(shifts),

            f"min_assigned_shifts_{role}",
        ...
        for member_index, member in enumerate(staff):
            ...
            weekend_count = model.NewIntVar(
                ...
                len(weekend_shift_indexes),

                f"weekend_shift_count_{member.id}",
        ...
        if weekend_counts:
            ...
            max_weekend_shifts = model.NewIntVar(

                0,

                len(weekend_shift_indexes),

                f"max_weekend_shifts_{role}",

            )

            min_weekend_shifts = model.NewIntVar(

                0,

                len(weekend_shift_indexes),

                f"min_weekend_shifts_{role}",
    ...
    if cuarto_turno_pattern_penalties:
        ...
        objective_terms.append(sum(cuarto_turno_pattern_penalties) * 40)



    # Soft constraints: balance shifts/weekends by role, respect requested days

    # off, and prefer the cuarto-turno cycle when operationally possible.
